...
class MQNAgent:
    ...
    def _build_model(self, learning_rate, momentum, min_grad):
        ...
        e = Flatten()(e)
        h = Dense(256, activation='linear')(e)
        m_val = Dense(256, activation='linear')(hist)
        m_key = Dense(256, activation='linear')(hist)
...
class SimpleMQNAgent:
    ...
    def _build_model(self, learning_rate, momentum, min_grad):
        ...
        e = Lambda(lambda x: x[:, -1, :])(features)
        h = Dense(16, activation='linear')(e)
        ...
        o = Dot(axes=1)([p, m_val])
        ho = Concatenate(axis=1)([h, o])
        q = Dense(32, activation='relu')(ho)
        q = Dense(self.action_size, activation='linear')(q)

        model = Model(inputs=input_obs, outputs=q)
        model.compile(loss=gym_util.huber_loss, optimizer=RMSprop(lr=learning_rate, rho=momentum, epsilon=min_grad))
        return model
