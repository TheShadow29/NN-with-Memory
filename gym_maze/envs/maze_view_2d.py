...
class MazeView2D:
    ...
    def __draw_color_blocks(self, transparency=160):
        # pdb.set_trace()
...
class Maze:
    ...
    def __set_random_colors(self, num_color_list):
        # find some random cells to break
        # num_portal_sets = int(num_portal_sets)
        # set_size = int(set_size)
        ...
        for i in range(num_color_list):
            # sample the set_size number of sell
            # pdb.set_trace()
            color_cell_id = random.sample(cell_ids, 1)[0]
            cell_color = random.sample(color_list, 1)[0]
            # color_locations = []
            # for color_cell_id in color_cell_ids:
            # remove the cell from the set of potential cell_ids
            cell_ids.pop(cell_ids.index(color_cell_id))
            color_list.pop(color_list.index(cell_color))
            # convert portal ids to location
            x = color_cell_id % self.MAZE_H
            y = int(color_cell_id / self.MAZE_H)
            # color_locations.append((x, y))
            # append the new portal to the maze
            color_location = (x, y)
            color = Color_cell(color_location, cell_color)
            self.__colors.append(color)
            self.__colors_dict[color_location] = color
            # create a dictionary of portals
            # for portal_location in portal_locations:
            # self.__portals_dict[portal_location] = portal
    ...
    def get_color(self, cell):
        if cell in self.__colors_dict:
            return self.__colors_dict[cell]
        return None
...
class Color_cell:
    ...
    def __init__(self, location, color):
        ...
        #     if isinstance(location, (tuple, list)):
        #         self.__locations.append(tuple(tuple(location), colors[ind]))
        #     else:
        #         raise ValueError("location must be a list or a tuple.")
