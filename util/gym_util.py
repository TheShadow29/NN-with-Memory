...
def init_state(obs, obs_shape, state_len, max_obs=None):
    if hasattr(obs_shape, '__len__'):
        ...
        processed_obs = to_categorical(obs, max_obs)
...
def add_obs(state, obs, obs_shape, max_obs=None):
    if hasattr(obs_shape, '__len__'):
        processed_obs = np.uint8(resize(rgb2gray(obs), obs_shape) * 255)
        ...
        processed_obs = to_categorical(obs, max_obs)
        processed_obs = processed_obs.flatten()
        processed_obs = processed_obs.reshape((1, processed_obs.shape[0]))
